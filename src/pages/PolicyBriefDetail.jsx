...
export default function PolicyBriefDetail() {
  ...
  const themeName = BRIEF_THEMES.find((t) => t.id === brief.theme)?.name ?? brief.theme
  ...
  const formattedDate = new Date(brief.published_at).toLocaleDateString("en-US", {
  ...
  return (
    <>
      <Helmet>
        <title>{brief.title} — Future Policy Lab</title>
        <meta name="description" content={brief.excerpt} />
        <meta property="og:title" content={brief.title} />
        <meta property="og:description" content={brief.excerpt} />
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-8">
        <div className="relative w-full aspect-video overflow-hidden rounded-2xl">
          <img src={brief.cover} alt={brief.title} className="w-full h-full object-cover" />
          ...
          <div className="absolute top-5 left-4 sm:left-8">
            ...
            >
              <ArrowLeft size={15} /> Policy Briefs
            </Link>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div className="leading-tight">
              ...
              <p className="text-xs">{brief.author.role}</p>
            </div>
          </div>
          ...
          <span className="flex items-center gap-1.5"><Clock size={13} /> {brief.read_time} min read</span>
        ...
        <div className={`my-8 p-6 rounded-2xl border ${
          ...
          <h2 className={`flex items-center gap-2 font-bold text-base mb-4 ${dark ? "text-emerald-400" : "text-emerald-700"}`}>
            <ListChecks size={18} /> Key Recommendations
          </h2>
      ...
      {related.length > 0 && (
        <div className={`border-t py-14 ${dark ? "border-white/8 bg-[#080d16]" : "border-slate-200 bg-slate-50"}`}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <h2 className={`text-xl font-bold mb-7 ${dark ? "text-white" : "text-slate-900"}`}>Related Policy Briefs</h2>
