...
const RU_DEBATE_COPY = {
  "should-cash-transfers-be-conditional": ["Должны ли денежные выплаты на образование зависеть от посещаемости?", "Условные выплаты обещают подотчётность, но не исключают ли они семьи, которые больше всего нуждаются в поддержке?"],
  "should-ai-eligibility-systems-be-banned": ["Следует ли запретить полностью автоматизированные решения о праве на социальные услуги?", "Полный запрет гарантирует проверку человеком, но может замедлить получение срочно необходимых льгот."],
  "should-informal-work-be-formalized": ["Должны ли правительства активно переводить неформальных работников в официальную занятость?", "Формализация обещает правовую защиту и налоговые поступления, но может не соответствовать реальному выбору молодых работников."],
  "should-water-allocation-be-locally-governed": ["Должны ли распределением воды управлять местные комитеты, а не центральные органы?", "Местное управление учитывает контекст, но может привести к непоследовательности и захвату решений наиболее активными участниками."],
  "should-budget-portals-be-mandatory": ["Должен ли каждый муниципалитет быть обязан законом публиковать портал открытого бюджета?", "Обязательное требование гарантирует базовый доступ к данным о расходах, но портал без информационной работы может стать лишь имитацией прозрачности."],
  "should-digital-literacy-be-mandatory-curriculum": ["Должна ли цифровая грамотность стать обязательной и оцениваемой частью национальной учебной программы?", "Обязательный статус гарантирует время на обучение, но оценка поднимает вопрос равного доступа к устройствам дома."],
}

const RU_POSITION_COPY = {
  "should-cash-transfers-be-conditional": [
    "Именно условность превращает выплату в политический инструмент, а не в разовую помощь. Связь с посещаемостью создаёт для школ общий стимул отслеживать учеников группы риска и поддерживать их.",
    "Условия наказывают семьи, которым труднее всего их выполнить: из-за болезни ребёнка, сломанного автобуса или сезонных работ. Контроль соблюдения часто стоит дороже, чем получаемый поведенческий эффект.",
  ],
  "should-ai-eligibility-systems-be-banned": [
    "Если система может без участия человека отказать в продовольственной помощи или жилье, цена для надлежащей процедуры слишком высока. Запрет заставит ведомства создать необходимую систему проверки человеком.",
    "Полный запрет смешивает хорошие и плохие решения. Автоматизация может быстрее обрабатывать простые продления и освобождать сотрудников для действительно спорных случаев. Нужны стандарты раскрытия и апелляции, а не отказ от автоматизации.",
  ],
  "should-informal-work-be-formalized": [
    "Официальная занятость даёт минимальные гарантии оплаты, безопасность труда и защиту от невыплаты. Если считать неформальность приемлемым постоянным состоянием, работников оставляют без защиты.",
    "Требования к формализации предполагают, что люди выбирают неформальность по незнанию, хотя издержки перехода действительно могут превышать выгоды. Переносимые льготы дают защиту без принудительного перехода.",
  ],
  "should-water-allocation-be-locally-governed": [
    "Центральные правила не могут учитывать местную сезонность, циклы выращивания и состав домохозяйств так, как местный комитет. Исследование показало, что местная координация сохраняет доступность воды.",
    "Местные комитеты могут быть захвачены наиболее активными семьями в ущерб тем, кто не может посещать собрания. Центральный надзор с местным участием безопаснее полной передачи контроля на места.",
  ],
  "should-budget-portals-be-mandatory": [
    "Законодательное требование устанавливает минимальный уровень, ниже которого муниципалитет не может опуститься. Даже несовершенный портал даёт жителям и журналистам основу для дальнейшей работы.",
    "Нефинансируемые требования создают формальные, но практически бесполезные порталы. Без бюджета на информационную работу портал превращается в проверку галочки и подрывает доверие к инвестициям в прозрачность.",
  ],
  "should-digital-literacy-be-mandatory-curriculum": [
    "Без обязательного и оцениваемого статуса цифровая грамотность уступает предметам с устоявшимися экзаменами и ясными стимулами. Оценка гарантирует необходимое время обучения для устранения разрыва между уверенностью и реальными навыками.",
    "Оценка предмета, сильно зависящего от доступа к устройствам дома, фактически оценивает доход семьи. Обязательное, но не оцениваемое обучение достигает цели доступа без усиления неравенства.",
  ],
}

for (const debate of debates) {
  const copy = RU_DEBATE_COPY[debate.slug]
  if (copy) {
    debate.motion_ru = copy[0]
    debate.excerpt_ru = copy[1]
    const positions = RU_POSITION_COPY[debate.slug]
    if (positions) {
      debate.forPosition.summary_ru = positions[0]
      debate.againstPosition.summary_ru = positions[1]
    }
  }
}
