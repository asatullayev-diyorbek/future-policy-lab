...
export default function ResearchDetail() {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    getResearchBySlug(slug).then(({ article: a, related: r }) => {
      if (cancelled) return
      setArticle(a)
      setRelated(r)
      setViews(a?.base_views ?? 0)
      setLoading(false)
      setSubmitted(false)
      window.scrollTo(0, 0)
      if (a) {
        recordView("research", a.slug, a.base_views).then(setViews)
        getComments("research", a.slug).then(setComments)
      }
    })
