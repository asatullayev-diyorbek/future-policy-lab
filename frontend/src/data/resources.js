...
const RU_RESOURCE_COPY = {
  "policy-brief-writing-template": {
    title: "Шаблон для написания политической записки", excerpt: "Структурированный шаблон, помогающий превратить исследовательские выводы в ясную рекомендацию для принятия решений.", format: "Шаблон (DOCX / Google Docs)",
    content: `Это тот же структурный шаблон, который редакционная команда использует при превращении результатов исследования в политическую записку. Он требует сначала сформулировать решение, аудиторию и рекомендацию.

## Как использовать

Заполните каждый раздел по порядку: контекст, доказательства, решение, рекомендация и путь реализации. Не переходите к следующему разделу, пока не сможете сформулировать решение одним предложением.

Шаблон предназначен для студентов, исследователей и авторов, которым нужно сделать сложные доказательства понятными для лиц, принимающих решения.`,
  },
  "root-cause-analysis-framework": {
    title: "Схема анализа первопричин", excerpt: "Рабочий лист для поиска причин проблемы до разработки политического решения.", format: "Рабочий лист (PDF)",
    content: `Слабые политические предложения часто направлены на симптом, а не на его причину. Этот рабочий лист использует адаптированную для политики структуру «пять почему», где причины редко бывают единственными.

## Как работать со схемой

Начните с наблюдаемой проблемы, последовательно задавайте вопрос «почему?» и фиксируйте доказательства для каждого предположения. Завершите работу проверкой того, на какие факторы предложенная мера действительно способна повлиять.`,
  },
  "rural-education-access-dataset": {
    title: "Набор данных о доступе к сельскому образованию (42 района)", excerpt: "Районные данные, лежащие в основе исследования различий в доступе к среднему образованию.", format: "CSV, 42 строки × 18 столбцов",
    content: `Это набор данных на уровне районов, использованный в исследовании «Устранение пробелов в доступе к среднему образованию», и опубликованный для повторного использования исследователями и студентами.

## Что включено

Данные охватывают расстояние до школы, посещаемость, зачисление, показатели отсева и характеристики программ поддержки. При использовании набора данных в публикациях укажите Future Policy Lab и ссылку на исходное исследование.`,
  },
  "municipal-trust-survey-data": {
    title: "Данные опроса о муниципальном доверии (12 муниципалитетов)", excerpt: "Панельные данные 2 400 жителей до и после запуска порталов открытого бюджета.", format: "CSV, 2 400 строк × 24 столбца",
    content: `Это данные панельного опроса, лежащие в основе исследования «Муниципальная прозрачность и общественное доверие». Они охватывают 2 400 жителей 12 муниципалитетов до запуска портала и через девять месяцев после него.

## Что включено

Набор содержит ответы по шкале институционального доверия, демографические показатели, потребление местных новостей и сведения об информационной работе муниципалитетов.`,
  },
  "getting-started-empirical-policy-research": {
    title: "Начало эмпирического исследования политики", excerpt: "Подборка материалов для первого самостоятельного исследования без требования продвинутой статистической подготовки.",
    content: `Несколько участников, готовящих первое самостоятельное исследование, попросили отправную точку, не предполагающую статистику уровня магистратуры. Эта подборка — наш ответ.

## С чего начать

Начните с методов опроса, затем переходите к причинному анализу и обзору литературы. Большинство предотвратимых ошибок студенческих исследований возникают на этапе дизайна, а не анализа.

Материалы рассчитаны на студентов и молодых исследователей, которые впервые планируют полевую или основанную на литературе работу.`,
  },
  "algorithmic-accountability-reading-list": {
    title: "Алгоритмическая подотчётность в государственном секторе", excerpt: "Список литературы о подотчётности алгоритмов при предоставлении государственных услуг.",
    content: `Список был подготовлен во время аудита автоматизированных систем определения права на льготы и охватывает более широкий ландшафт алгоритмической подотчётности в государственных услугах.

## С чего начать

Если вы только знакомитесь с темой, начните с руководства Data & Society: оно короче и объясняет ключевые понятия, которые предполагают книги.

## Связанное исследование

Исследование Future Policy Lab «Алгоритмическое принятие решений в государственных услугах» применяет эти идеи к аудиту девяти систем и предлагает конкретный стандарт раскрытия причин.`,
  },
}

for (const resource of resources) {
  const copy = RU_RESOURCE_COPY[resource.slug]
  if (copy) {
    resource.title_ru = copy.title
    resource.excerpt_ru = copy.excerpt
    resource.content_ru = copy.content
    if (copy.format) resource.format_ru = copy.format
  }
}
